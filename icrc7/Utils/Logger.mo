...
module {
  ...
  public class Logger<A>(s : State<A>) {
    ...
    public func view(from : Nat, to : Nat) : View<A> {
      ...
      if (i + s.num_of_lines > from and i <= to) {
        ...
        var k = if (i < from) { Nat.sub(from, i) } else { 0 };
        let m = if (i + s.num_of_lines > to) { Nat.sub(to + 1, i) } else {
          s.num_of_lines;
        };
        while (k < m) {
          buf.add(arr[k]);
  ...
  public func test() : Suite {
    ...
    let append_test = Suite.suite(
      "append",
      Array.tabulate<Suite>(
        N,
        func(n : Nat) : Suite {
          logger.append([n]);
          Suite.test(
            Text.concat("append/", Nat.toText(n)),
    ...
    for (i in Iter.range(0, N - 2)) {
      for (j in Iter.range(i, N - 1)) {
        view_tests.add(
          Suite.test(
            Text.join("/", Iter.fromArray(["view", Nat.toText(i), Nat.toText(j)])),
